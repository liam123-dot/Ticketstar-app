...
export default function ForgotPasswordScreen({navigation}) {
  ...
  const combinedRegex = /^([^\s@]+@[^\s@]+\.[^\s@]+)|(\+44\d{10})$/;
  ...
  return (
    <SafeAreaView style={{flex: 1}}>
      ...
      <View
        ...
        {userDoesNotExists ? (
          <Text>
            The email or phone number you submitted is not registered with an
            existing user
          </Text>
        ) : codeSent ? (
          <>
            <Text>
              A code has been sent to your provided phone number or email
            </Text>
            <InputField
